...
def clipList(current,prev):
    ...
    return ee.Dictionary({'imlist':imlist,'poly':poly,'enl':enl,'ctr':ctr.add(1)})  
...
def on_collect_button_clicked(b):
    ...
    '''
    global result, count, timestamplist, uniquetimestamps, archive_crs
    with w_out:
        try:
            w_out.clear_output()
            clear_layers()
...
#           make timestamps in YYYYMMDD format       
            ...
            print('Time series length %i'%len(uniquetimestamps)) 
            ...
            cList = collection.map(get_vvvh).toList(500)   
            ...
            first = ee.Dictionary({'imlist':ee.List([]),'enl':ee.Number(4.4),'poly':poly,'ctr':ee.Number(0)})          
