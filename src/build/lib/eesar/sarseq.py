...
import ee
from sarseqalgorithm import change_maps
...
#   """Export the bitemporal change maps as images with properties.
# 
#     Ensure that 'system:time_start' and 'system:time_end' are added to the
#     images that are being added to an existing ImageCollection.
# 
#   Args:
#     result: ee.Dictionary of images
#     assetpath: The imagecollection where images will be written to
#     acquisition_times: ee.List of images that were processed
#     poly: The bounding-box specifying the area of interest
#   """
# 
#   def tag_images(i):
#     i_num = ee.Number(i)
#     image = bmap.select(i_num)
#     return image
# 
#   bmap = ee.Image(result.get('bmap')).byte()
#   size = bmap.bandNames().size()
#   images = ee.List.sequence(0, size.add(-1))  # pytype: disable=attribute-error
#   tagged_images = ee.List(images.map(tag_images))
#   for i in range(size.getInfo() - 1):
#     asset_id = f'{assetpath}/bmap{acquisition_times[i]}'
#     image = ee.Image(tagged_images.get(i)).clip(poly)
#     image = image.set('system:time_start', acquisition_times[i]).set(
#         'system:time_end',
#         acquisition_times[i + 1]).set('system:image_id', ee.String('bmap'))
#     asset_export = ee.batch.Export.image.toAsset(
#         image,
...
#         maxPixels=1e11)
#     asset_export.start()
#     print(f'Exporting change maps to {asset_id} task id:{asset_export.id}')
